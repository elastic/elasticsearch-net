...
while (searchResponse.Documents.Any()) <2>
{
    ...
    searchResponse = Client.Scroll<Project>("10s", searchResponse.ScrollId); <4>
}
----
...
<3> do something with the response, (Client search request also return the first iteration of query)
<4> Get next page scroll response and set it to searchReponse to use it on ProcessResponse
