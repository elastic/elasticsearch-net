...
<2> how many retries are attempted
...
client.BulkAll(people, b => b
      .RetryDocumentPredicate((item, person) => <2>
      {
...
client.BulkAll(people, b => b
      .DroppedDocumentCallback((item, person) => <3>
      {
...
<2> decide if a document should be retried in the event of a failure

<3> if a document cannot be indexed this delegate is called
