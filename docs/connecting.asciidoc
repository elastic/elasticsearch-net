...
By default, with security features like authentication and TLS enabled. When you 
start {es} for the first time, TLS is configured automatically for the HTTP 
layer. A CA certificate is generated and stored on disk that is used to sign the 
...
mechanisms for working with certificates, but the simplest choice is to use the 
