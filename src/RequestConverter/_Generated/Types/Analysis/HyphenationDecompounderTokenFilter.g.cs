// Licensed to Elasticsearch B.V under one or more agreements.
// Elasticsearch B.V licenses this file to you under the Apache 2.0 License.
// See the LICENSE file in the project root for more information.
//
// ███╗   ██╗ ██████╗ ████████╗██╗ ██████╗███████╗
// ████╗  ██║██╔═══██╗╚══██╔══╝██║██╔════╝██╔════╝
// ██╔██╗ ██║██║   ██║   ██║   ██║██║     █████╗
// ██║╚██╗██║██║   ██║   ██║   ██║██║     ██╔══╝
// ██║ ╚████║╚██████╔╝   ██║   ██║╚██████╗███████╗
// ╚═╝  ╚═══╝ ╚═════╝    ╚═╝   ╚═╝ ╚═════╝╚══════╝
// ------------------------------------------------
//
// This file is automatically generated.
// Please do not edit these files manually.
//
// ------------------------------------------------

#nullable restore

using Elastic.Clients.Elasticsearch.Serialization;
using System;
using System.Linq;

namespace Elastic.Clients.Elasticsearch.Analysis;

public partial class HyphenationDecompounderTokenFilter : RequestConverter.ICodeFormattable
{
	public void FormatCode(System.Text.StringBuilder sb)
	{
		var instance = this;
		sb.Append("new()");
		var hasProps = false;
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("HyphenationPatternsPath = ");
			sb.Append("\"");
			sb.Append(instance.HyphenationPatternsPath);
			sb.Append("\"");
		}

		if (instance.MaxSubwordSize is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("MaxSubwordSize = ");
			sb.Append(instance.MaxSubwordSize.Value);
		}

		if (instance.MinSubwordSize is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("MinSubwordSize = ");
			sb.Append(instance.MinSubwordSize.Value);
		}

		if (instance.MinWordSize is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("MinWordSize = ");
			sb.Append(instance.MinWordSize.Value);
		}

		if (instance.NoOverlappingMatches is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("NoOverlappingMatches = ");
			sb.Append(instance.NoOverlappingMatches.Value ? "true" : "false");
		}

		if (instance.NoSubMatches is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("NoSubMatches = ");
			sb.Append(instance.NoSubMatches.Value ? "true" : "false");
		}

		if (instance.OnlyLongestMatch is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("OnlyLongestMatch = ");
			sb.Append(instance.OnlyLongestMatch.Value ? "true" : "false");
		}

		if (instance.Version is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("Version = ");
			sb.Append("\"");
			sb.Append(instance.Version);
			sb.Append("\"");
		}

		if (instance.WordList is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("WordList = ");
			sb.Append("[");
			RequestConverter.CodeFormatter.FormatCode(instance.WordList, (item, sb) => { sb.Append("\""); sb.Append(item); sb.Append("\""); }, sb);
			sb.Append("]");
		}

		if (instance.WordListPath is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("WordListPath = ");
			sb.Append("\"");
			sb.Append(instance.WordListPath);
			sb.Append("\"");
		}

		if (hasProps)
			sb.Append(" }");
	}
}