...
public partial class CompositeAggregate : RequestConverter.ICodeFormattable
{
	...
	{
		var initializer = writer.BeginObjectInitializer("CompositeAggregate", true);
		if (AfterKey is not null)
		{
			...
			writer.Write(">() ");
			writer.WriteInlineList(AfterKey, (w, kvp) => { w.Write("{ "); w.WriteString(kvp.Key); w.Write(", "); kvp.Value.FormatCode(w); w.Write(" }"); }, "{ ", " }", ", ");
		}
