// Licensed to Elasticsearch B.V under one or more agreements.
// Elasticsearch B.V licenses this file to you under the Apache 2.0 License.
// See the LICENSE file in the project root for more information.
//
// ███╗   ██╗ ██████╗ ████████╗██╗ ██████╗███████╗
// ████╗  ██║██╔═══██╗╚══██╔══╝██║██╔════╝██╔════╝
// ██╔██╗ ██║██║   ██║   ██║   ██║██║     █████╗
// ██║╚██╗██║██║   ██║   ██║   ██║██║     ██╔══╝
// ██║ ╚████║╚██████╔╝   ██║   ██║╚██████╗███████╗
// ╚═╝  ╚═══╝ ╚═════╝    ╚═╝   ╚═╝ ╚═════╝╚══════╝
// ------------------------------------------------
//
// This file is automatically generated.
// Please do not edit these files manually.
//
// ------------------------------------------------

#nullable restore

using Elastic.Clients.Elasticsearch.Serialization;
using System;
using System.Linq;

namespace Elastic.Clients.Elasticsearch.Ingest;

public partial class UserAgentProcessor : RequestConverter.ICodeFormattable
{
	public void FormatCode(System.Text.StringBuilder sb)
	{
		var instance = this;
		sb.Append("new()");
		var hasProps = false;
		if (instance.Description is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("Description = ");
			sb.Append("\"");
			sb.Append(instance.Description);
			sb.Append("\"");
		}

		if (instance.ExtractDeviceType is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("ExtractDeviceType = ");
			sb.Append(instance.ExtractDeviceType.Value ? "true" : "false");
		}

		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("Field = ");
			instance.Field.FormatCode(sb);
		}

		if (instance.If is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("If = ");
			instance.If.FormatCode(sb);
		}

		if (instance.IgnoreFailure is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("IgnoreFailure = ");
			sb.Append(instance.IgnoreFailure.Value ? "true" : "false");
		}

		if (instance.IgnoreMissing is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("IgnoreMissing = ");
			sb.Append(instance.IgnoreMissing.Value ? "true" : "false");
		}

		if (instance.OnFailure is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("OnFailure = ");
			sb.Append("[");
			RequestConverter.CodeFormatter.FormatCode(instance.OnFailure, (item, sb) => { item.FormatCode(sb); }, sb);
			sb.Append("]");
		}

		if (instance.Properties is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("Properties = ");
			sb.Append("[");
			RequestConverter.CodeFormatter.FormatCode(instance.Properties, (item, sb) => { Elastic.Clients.Elasticsearch.Ingest.UserAgentPropertyCodeFormatter.FormatCode(item, sb); }, sb);
			sb.Append("]");
		}

		if (instance.RegexFile is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("RegexFile = ");
			sb.Append("\"");
			sb.Append(instance.RegexFile);
			sb.Append("\"");
		}

		if (instance.Tag is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("Tag = ");
			sb.Append("\"");
			sb.Append(instance.Tag);
			sb.Append("\"");
		}

		if (instance.TargetField is not null)
		{
			sb.Append(hasProps ? ", " : " { ");
			hasProps = true;
			sb.Append("TargetField = ");
			instance.TargetField.FormatCode(sb);
		}

		if (hasProps)
			sb.Append(" }");
	}
}