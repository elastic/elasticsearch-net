...
public partial class ShardStats : RequestConverter.ICodeFormattable
{
	...
	{
		var initializer = writer.BeginObjectInitializer("Elastic.Clients.Elasticsearch.IndexManagement.ShardStats", false);
		if (Bulk is not null)
		{
		...
		if (Shards is not null)
		{
			...
			writer.Write(">()");
			writer.WriteBlockList(Shards, (w, kvp) => { w.Write("{ "); w.WriteString(kvp.Key); w.Write(", "); w.WriteObjectValue(kvp.Value); w.Write(" }"); });
		}
