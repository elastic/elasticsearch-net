...
public partial class TrainedModelConfig : RequestConverter.ICodeFormattable
{
	...
	{
		var initializer = writer.BeginObjectInitializer("Elastic.Clients.Elasticsearch.MachineLearning.TrainedModelConfig", false);
		...
		if (DefaultFieldMap is not null)
		{
			...
			writer.WriteTypeRef("string");
			writer.Write(">()");
			writer.WriteBlockList(DefaultFieldMap, (w, kvp) => { w.Write("{ "); w.WriteString(kvp.Key); w.Write(", "); w.WriteString(kvp.Value); w.Write(" }"); });
		}
