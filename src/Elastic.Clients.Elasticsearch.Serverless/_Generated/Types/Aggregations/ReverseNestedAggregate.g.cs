...
internal sealed partial class ReverseNestedAggregateConverter : JsonConverter<ReverseNestedAggregate>
{
	public override ReverseNestedAggregate Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
	{
		...
		return new ReverseNestedAggregate { Aggregations = new Elastic.Clients.Elasticsearch.Serverless.Aggregations.AggregateDictionary(additionalProperties), DocCount = docCount, Meta = meta };
	}

	public override void Write(Utf8JsonWriter writer, ReverseNestedAggregate value, JsonSerializerOptions options)
	{
		throw new NotImplementedException("'ReverseNestedAggregate' is a readonly type, used only on responses and does not support being written to JSON.");
	}
}
