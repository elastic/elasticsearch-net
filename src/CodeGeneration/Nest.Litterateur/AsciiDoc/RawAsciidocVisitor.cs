				...
				if (includeAttribute != null)
				{
					...
					foreach (var directory in directories)
					{
						foreach (var file in Directory.EnumerateFiles(Path.Combine(Program.OutputDirPath, directory), "*usage.asciidoc", SearchOption.AllDirectories))
						{
							...
							var listItem = new UnorderedListItem
							{
								Elements =
								{
									new Paragraph(
										new InternalAnchor(fileNameWithoutExtension, fileNameWithoutExtension.LowercaseHyphenToPascal()))
								}
							};

							open.Elements.Add(listItem);
						}
					}
				}
			}
