...
namespace Elasticsearch.Net
{
	public class InMemoryConnection : IConnection
	{
		...
		public virtual ElasticsearchResponse<TReturn> Request<TReturn>(RequestData requestData) where TReturn : class =>
			ReturnConnectionStatus<TReturn>(requestData);

		public virtual async Task<ElasticsearchResponse<TReturn>> RequestAsync<TReturn>(RequestData requestData, CancellationToken cancellationToken)
			where TReturn : class =>
			await ReturnConnectionStatusAsync<TReturn>(requestData, cancellationToken).ConfigureAwait(false);

		void IDisposable.Dispose() => DisposeManagedResources();

		protected ElasticsearchResponse<TReturn> ReturnConnectionStatus<TReturn>(RequestData requestData, byte[] responseBody = null,
			int? statusCode = null
		)
			where TReturn : class
		{
		...
		protected async Task<ElasticsearchResponse<TReturn>> ReturnConnectionStatusAsync<TReturn>(RequestData requestData,
			CancellationToken cancellationToken, byte[] responseBody = null, int? statusCode = null
		)
			where TReturn : class
		{
			var body = responseBody ?? _responseBody;
