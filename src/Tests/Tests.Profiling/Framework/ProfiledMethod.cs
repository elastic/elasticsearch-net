...
namespace Tests.Profiling.Framework
{
	public class ProfiledMethod
	{
		...
		{
			if (MethodInfo.ReturnType != typeof(Task)) throw new ArgumentException("MethodInfo must return Task");
		...
		{
			if (MethodInfo.ReturnType != typeof(void)) throw new ArgumentException("MethodInfo must return void");
