...
namespace Tests.Cat.CatSegments
{
	...
	public class CatSegmentsApiTests : ApiTestBase<ICatResponse<CatSegmentsRecord>, ICatSegmentsRequest, CatSegmentsDescriptor, CatSegmentsRequest>
	{
		...
		[I] public async Task HasSegments() => await this.AssertOnAllResponses(r =>
		{
			r.Records.Should().NotBeEmpty().And.Contain(a => !string.IsNullOrEmpty(a.Index));
		});
