...
namespace Nest.Tests.Integration.Core
{
	...
	public class IndexTests : IntegrationTests
	{
		...
		public void IndexUsingCreateFlag()
		{
			...
			// Index the document
			var indexResult = this.Client.Index<ElasticsearchProject>(doc, i => i.OpType(OpTypeOptions.Create));
			...
			var response = this.Client.Index<ElasticsearchProject>(doc, i => i.OpType(OpTypeOptions.Create));
		...
		public void IndexUsingCreateFlagOnNoRawResponseClient()
		{
			...
			// Index the document
			var indexResult = this.ClientNoRawResponse.Index<ElasticsearchProject>(doc, i => i.OpType(OpTypeOptions.Create));
			indexResult.IsValid.Should().BeTrue();
			...
			// Now try to index the document again while using the Create Flag
			var response = this.ClientNoRawResponse.Index<ElasticsearchProject>(doc, i => i.OpType(OpTypeOptions.Create));
