...
namespace Tests.Core.Serialization
{
	...
	public class ObjectRoundTripper<T> : RoundTripperBase
	{
		...
		internal ObjectRoundTripper(T @object,
			...
			: base(settingsModifier, serializerFactory, false) => _object = @object;
	...
	public class JsonRoundTripper : RoundTripperBase
	{
		...
		internal JsonRoundTripper(
			...
			: base(settingsModifier, serializerFactory, preserveNullInExpected) => _expectedJson = expectedJson;
		...
		{
			var jo = JObject.Parse(Tester.Client.Serializer.SerializeToString(document));
