...
namespace Tests.ClientConcepts.ConnectionPooling.Sniffing
{
	public class RoleDetection
	{
		...
		*/
		...
		public async Task SkipMasterOnlyNodes()
		{
			...
			seenNodes.Should()
				.HaveCount(totalNodesInTheCluster
					- masterNodes.Length); // <1> `seenNodes` is a hash set of all the ports we hit. assert that this is equal to `known total nodes - known master only nodes`
		}
		...
		public async Task RespectsCustomPredicate()
		{
			...
			)
			{
				AssertPoolAfterStartup =
					pool => // <3> After sniffing on startup, assert that the pool of nodes that the client will execute API calls against only contains the three nodes that are in `rack_one`
					{
						pool.Should().NotBeNull();
						var nodes = pool.CreateView().ToList();
	...
	public class RealWorldRoleDetection : IClusterFixture<SniffRoleDetectionCluster>
	{
		...
		private RequestPipeline CreatePipeline()
		{
			...
			var pipeline = new RequestPipeline(_settings, DateTimeProvider.Default, new MemoryStreamFactory(),
