		...
		*/
		...
		public async Task OnRequestCompletedIsCalledWhenExceptionIsThrown()
		{
			...
			Assert.Throws<ElasticsearchClientException>(() =>
				client.RootNodeInfo()); // <3> Assert an exception is thrown and the counter is incremented
			counter.Should().Be(1);
		...
		public async Task UsingOnRequestCompletedForLogging()
		{
			...
			var settings =
				new ConnectionSettings(connectionPool,
						new InMemoryConnection()) // <1> Here we use `InMemoryConnection` but in a real application, you'd use an `IConnection` that _actually_ sends the request, such as `HttpConnection`
					.DefaultIndex("default-index")
					.DisableDirectStreaming() // <2> Disable direct streaming so we can capture the request and response bytes
					.OnRequestCompleted(
						apiCallDetails => // <3> Perform some action when a request completes. Here, we're just adding to a list, but in your application you may be logging to a file.
						{
