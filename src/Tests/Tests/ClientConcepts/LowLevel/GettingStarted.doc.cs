...
namespace Tests.ClientConcepts.LowLevel
{
	...
	public class GettingStarted : IClusterFixture<WritableCluster>
	{
		...
		private readonly IElasticLowLevelClient lowlevelClient =
			new ElasticLowLevelClient(new ConnectionConfiguration(new SingleNodeConnectionPool(new Uri("http://localhost:9200")),
		...
		public async Task Indexing()
		{
			...
			var indexResponse =
				lowlevelClient.Index<BytesResponse>("people", "person", "1",
					PostData.Serializable(person)); //<1> synchronous method that returns an `IIndexResponse`
			byte[] responseBytes = indexResponse.Body;

			var asyncIndexResponse =
				await lowlevelClient.IndexAsync<StringResponse>("people", "person", "1",
					PostData.Serializable(person)); //<2> asynchronous method that returns a `Task<IIndexResponse>` that can be awaited
		...
		*/
		public void ResponseProperties()
		{
			...
			var successOrKnownError =
				searchResponse
					.SuccessOrKnownError; // <2> Response is successful, or has a response code between 400-599 that indicates the request cannot be retried.
		...
		*/
		...
		public void FineGrainedControl()
		{
			var settings = new ConnectionConfiguration(new Uri("http://example.com:9200"))
				...
				{
					if (apiCallDetails.HttpStatusCode == 418) throw new TimeForACoffeeException();
				});
