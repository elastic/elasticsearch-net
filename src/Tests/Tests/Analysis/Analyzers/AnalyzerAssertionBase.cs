...
namespace Tests.Analysis.Analyzers
{
	...
	public abstract class AnalyzerAssertionBase<TAssertion> where TAssertion : AnalyzerAssertionBase<TAssertion>, new()
	{
		...
		protected AnalyzerAssertionBase()
		{
		...
		protected abstract IAnalyzer Initializer { get; }
		protected abstract Func<string, AnalyzersDescriptor, IPromise<IAnalyzers>> Fluent { get; }
		...
		private static CreateIndexRequest InitializerCall(string index) => new CreateIndexRequest(index)
		{
			Settings = new IndexSettings
			{
				...
				{
					Analyzers = new Nest.Analyzers { { AssertionSetup.Name, AssertionSetup.Initializer } }
		...
		private static Func<CreateIndexDescriptor, ICreateIndexRequest> FluentCall => i => i
			.Settings(s => s
				.Analysis(a => a
					.Analyzers(d => AssertionSetup.Fluent(AssertionSetup.Name, d))
				)
			);
