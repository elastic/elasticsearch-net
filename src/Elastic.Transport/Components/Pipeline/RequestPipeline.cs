...
namespace Elastic.Transport
{
	public class RequestPipeline : IRequestPipeline
	{
		...
		public ClientException CreateClientException<TResponse>(
			...
			where TResponse : class, ITransportResponse, new()
		{
			...
			var clientException = new ClientException(pipelineFailure, exceptionMessage, innerException)
			{
				Request = data,
		...
		public async Task FirstPoolUsageAsync(SemaphoreSlim semaphore, CancellationToken cancellationToken)
		{
			...
			// everywhere else it would bubble out wrapped in a `UnexpectedClientException`
		...
		public void Sniff()
		{
			...
			foreach (var node in SniffNodes)
			{
				...
				using(DiagnosticSource.Diagnose(DiagnosticSources.RequestPipeline.Sniff, requestData))
				{
					...
					{
						audit.Path = requestData.PathAndQuery;
						var response = _connection.Request<ElasticsearchSniffResponse>(requestData);
						d.EndState = response;
		...
		public async Task SniffAsync(CancellationToken cancellationToken)
		{
			...
			foreach (var node in SniffNodes)
			{
				...
				using (var d = DiagnosticSource.Diagnose<RequestData, IApiCallDetails>(DiagnosticSources.RequestPipeline.Sniff, requestData))
				{
					...
					{
						audit.Path = requestData.PathAndQuery;
						var response = await _connection.RequestAsync<ElasticsearchSniffResponse>(requestData, cancellationToken).ConfigureAwait(false);
						d.EndState = response;
		...
		{
			var clientException = new ClientException(PipelineFailure.NoNodesAttempted, NoNodesAttemptedMessage, (Exception)null);
			using (Audit(NoNodesAttempted))
				throw new UnexpectedClientException(clientException, seenExceptions)
				{
					Request = requestData,
					AuditTrail = AuditTrail
				};
