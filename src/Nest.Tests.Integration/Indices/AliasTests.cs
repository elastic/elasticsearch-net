...
namespace Nest.Tests.Integration.Indices
{
	...
	public class AliasTest : IntegrationTests
	{
		...
		public void SimpleAddRemoveAlias()
		{
			...
			var count1 = this._client.Count<dynamic>(c => c.Index(index).Query(q => q.MatchAll()));
			var count2 = this._client.Count<dynamic>(c => c.Index(alias).Query(q => q.MatchAll()));
			Assert.AreEqual(count1.Count, count2.Count);
			r = this._client.Alias(a=>a.Remove(o=>o.Index(index).Alias(alias)));
			count1 = this._client.Count<dynamic>(c=>c.Index(index).Query(q=>q.MatchAll()));
			count2 = this._client.Count<dynamic>(c=>c.Index(alias).Query(q=>q.MatchAll()));
		...
		public void SimpleRenameAlias()
		{
			...
			count1 = this._client.Count<dynamic>(c=>c.Index(index).Query(q=>q.MatchAll()));
			count2 = this._client.Count<dynamic>(c=>c.Index(alias).Query(q=>q.MatchAll()));
			var count3 = this._client.Count<dynamic>(c=>c.Index(renamed).Query(q=>q.MatchAll()));
			...
			count1 = this._client.Count<dynamic>(c=>c.Index(index).Query(q=>q.MatchAll()));
			count3 = this._client.Count<dynamic>(c=>c.Index(renamed).Query(q=>q.MatchAll()));
