...
namespace Nest.Tests.Unit.QueryJson.ConditionLessStrict
{
	...
	public class SemiStrictBoolTests : BaseJsonTests
	{
		...
		{
			this.DoSemiStrictBoolQuery(q => q.Strict().Terms(p => p.Name, new string[] { _c.Name1 }) && q.Term(p => p.Name, _c.Name1));
			this.DoSemiStrictBoolQuery(q => q.Terms(p => p.Name, new string[] { _c.Name1 }) || q.Strict().Term(p => p.Name, _c.Name1));
		}
		[Test]
		public void BothStrict()
		{
			this.DoSemiStrictBoolQuery(q => q.Strict().Terms(p => p.Name, new string[] { _c.Name1 }) && q.Strict().Term(p => p.Name, _c.Name1));
			this.DoSemiStrictBoolQuery(q => q.Strict().Terms(p => p.Name, new string[] { _c.Name1 }) || q.Strict().Term(p => p.Name, _c.Name1));
		}
		[Test]
		public void OneStrictOneNotValid()
		{
			this.DoValidSemiStrictBoolQuery(q => q.Strict().Terms(p => p.Name, new string[] { "term" }) && q.Term(p => p.Name, _c.Name1));
			this.DoValidSemiStrictBoolQuery(q => q.Terms(p => p.Name, new string[] { _c.Name1 }) || q.Strict().Term(p => p.Name, "test"));
		}
		[Test]
