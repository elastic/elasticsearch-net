...
namespace Nest
{
	...
	public class AnalyzeTokenFiltersDescriptor : DescriptorPromiseBase<AnalyzeTokenFiltersDescriptor, AnalyzeTokenFilters>
	{
		...
		/// <summary>
		///     Collations are used for sorting documents in a language-specific word order. The icu_collation token filter is available to all indices
		///     and defaults to using the DUCET collation, which is a best-effort attempt at language-neutral sorting.
		...
		/// <summary>
		///     Overrides stemming algorithms, by applying a custom mapping, then protecting these terms from being modified by stemmers. Must be
		///     placed before any stemming filters.
		/// </summary>
