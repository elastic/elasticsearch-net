...
namespace Nest
{
    public partial class ElasticClient
    {
        ...
        public IIndexResponse Index<T>(T @object, string indexName)
            where T : class
        {
            @object.ThrowIfNull("object");
            var indexSelector = !string.IsNullOrEmpty(indexName) ? (Func<IndexDescriptor<T>, IndexDescriptor<T>>)(x => x.Index(indexName)) : (s => s);
            var descriptor = indexSelector(new IndexDescriptor<T>().Object(@object));
            return this.Dispatch<IndexDescriptor<T>, IndexRequestParameters, IndexResponse>(
                descriptor,
                (p, d) => this.RawDispatch.IndexDispatch<IndexResponse>(p, @object));
