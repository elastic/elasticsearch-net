...
namespace Nest
{
		...
		: DescriptorBase<DFRSimilarityDescriptor, IDFRSimilarity>, IDFRSimilarity
	{
		...
		float? IDFRSimilarity.NormalizationH1C { get; set; }
		float? IDFRSimilarity.NormalizationH2C { get; set; }
		float? IDFRSimilarity.NormalizationH3C { get; set; }
		float? IDFRSimilarity.NormalizationZZ { get; set; }
		...
		public DFRSimilarityDescriptor NormalizationH1(float c) => Assign(a =>
		{
		...
		/// </summary>
		/// <param name="c">hyper-parameter that controls the term frequency normalization with respect to the document length.</param>
		public DFRSimilarityDescriptor NormalizationH2(float c) => Assign(a =>
		{
		...
		public DFRSimilarityDescriptor NormalizationH3(float mu) => Assign(a =>
		{
		...
		public DFRSimilarityDescriptor NormalizationZ(float z) => Assign(a =>
		{
