...
namespace Nest
{
	internal class IndexSettingsConverter : JsonConverter
	{
		...
		public void Set<T>(IDictionary<string, JProperty> settings, string key, Action<T> assign)
		{
			if (!settings.ContainsKey(key)) return;
			var v = settings[key];
			assign(v.Value.ToObject<T>());
		}
		...
		public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
		{
			...
			var settings = Flatten(JObject.Load(reader)).Properties().ToDictionary(p => p.Name, p => p);
			foreach (var kv in settings)
			{
				...
				else
				{
					...
					Set<int?>(settings, UpdatableSettings.RecoveryInitialShards, v => s.RecoveryInitialShards = v);
					...
					Set<SlowLogLevel?>(settings, UpdatableSettings.SlowlogIndexingThresholdFetchTrace, v => indexing.LogLevel = v);
					Set<int?>(settings, UpdatableSettings.SlowlogIndexingThresholdFetchTrace, v => indexing.Source = v);
