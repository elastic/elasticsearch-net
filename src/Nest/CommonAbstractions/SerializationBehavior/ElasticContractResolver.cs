...
namespace Nest
{
	public class ElasticContractResolver : DefaultContractResolver
	{
		...
		{
			_contractConverters = contractConverters;
		...
		protected override JsonContract CreateContract(Type objectType) => ConnectionSettings.Inferrer.Contracts.GetOrAdd(objectType, o =>
		{
			...
			if (_contractConverters.HasAny())
			{
				foreach (var c in _contractConverters)
				{
					var converter = c(o);
					if (converter == null)
						continue;
		...
		protected override JsonProperty CreateProperty(MemberInfo member, MemberSerialization memberSerialization)
		{
				...
				&& typeof(IEnumerable).IsAssignableFrom(property.PropertyType))
			{
				...
				property.ShouldSerialize =
					property.ShouldSerialize == null ? shouldSerialize : o => property.ShouldSerialize(o) && shouldSerialize(o);
			}

			IPropertyMapping propertyMapping = null;
			if (!ConnectionSettings.PropertyMappings.TryGetValue(member, out propertyMapping))
				propertyMapping = ElasticsearchPropertyAttributeBase.From(member);

			var serializerMapping = ConnectionSettings.Serializer?.CreatePropertyMapping(member);
