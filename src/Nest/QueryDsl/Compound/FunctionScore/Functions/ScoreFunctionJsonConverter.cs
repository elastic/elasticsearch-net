...
namespace Nest
{
	public class ScoreFunctionJsonConverter : JsonConverter
	{
			...
			writer.WriteStartObject();
			{
				...
				writer.WriteStartObject();
				{
					var write = WriteNumericDecay(writer, decay as IDecayFunction<double?, double?>, serializer)
								|| WriteDateDecay(writer, decay as IDecayFunction<DateMath, TimeUnit>, serializer)
								|| WriteGeoDecay(writer, decay as IDecayFunction<GeoLocation, DistanceUnit>, serializer);
		...
		private bool WriteDateDecay(JsonWriter writer, IDecayFunction<DateMath, TimeUnit> value, JsonSerializer serializer)
		{
		...
		private bool WriteGeoDecay(JsonWriter writer, IDecayFunction<GeoLocation, DistanceUnit> value, JsonSerializer serializer)
		{
			if (value == null || value.Field.IsConditionless()) return false;
			writer.WritePropertyName("origin");
			serializer.Serialize(writer, value.Origin);
