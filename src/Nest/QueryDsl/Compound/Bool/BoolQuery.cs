...
namespace Nest
{
	...
	public interface IBoolQuery : IQuery
	{
		...
		/// <summary>
		///     The clause (query) should appear in the matching document. A boolean query with no must clauses, one or more should clauses must match
		///     a document.
	...
	public class BoolQuery : QueryBase, IBoolQuery
	{
		...
		/// <summary>
		///     The clause (query) must not appear in the matching documents. Note that it is not possible to search on documents that only consists of
		///     a must_not clauses.
		/// </summary>
