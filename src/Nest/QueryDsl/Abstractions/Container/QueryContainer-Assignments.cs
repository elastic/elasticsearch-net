...
#pragma warning disable 618
		IGeoDistanceRangeQuery IQueryContainer.GeoDistanceRange
		{
			get => _geoDistanceRange;
			set => _geoDistanceRange = Set(value);
		}
#pragma warning restore 618
		IGeoHashCellQuery IQueryContainer.GeoHashCell
		{
			get => _geoHashCell;
			set => _geoHashCell = Set(value);
		}
...
#pragma warning disable 618
		IIndicesQuery IQueryContainer.Indices
		{
			get => _indices;
...
#pragma warning disable 618
		ITemplateQuery IQueryContainer.Template
		{
			get => _template;
			set => _template = Set(value);
		}
#pragma warning restore 618
		...
		private T Set<T>(T value) where T : IQuery
		{
			if (ContainedQuery != null)
				throw new Exception($"QueryContainer can only hold a single query already contains a {ContainedQuery.GetType().Name}");
