...
namespace Nest
{
	...
	public partial class ElasticClient
	{
		...
		public ICreateResponse Create<T>(T document, Func<CreateDescriptor<T>, ICreateRequest> selector = null) where T : class
		{
			...
			return Create(request ?? selector.InvokeOrDefault(new CreateDescriptor<T>(document)));
		}

		/// <inheritdoc />
		public ICreateResponse Create(ICreateRequest request) =>
			Dispatcher.Dispatch<ICreateRequest, CreateRequestParameters, CreateResponse>(
				request,
				LowLevelDispatch.CreateDispatch<CreateResponse>
			);

		/// <inheritdoc />
		public Task<ICreateResponse> CreateAsync<T>(T document, Func<CreateDescriptor<T>, ICreateRequest> selector = null,
			CancellationToken cancellationToken = default(CancellationToken)
		) where T : class
		{
			var request = document as ICreateRequest;
			return CreateAsync(request ?? selector.InvokeOrDefault(new CreateDescriptor<T>(document)), cancellationToken);
		}
		...
		public Task<ICreateResponse> CreateAsync(ICreateRequest request, CancellationToken cancellationToken = default(CancellationToken)) =>
			Dispatcher.DispatchAsync<ICreateRequest, CreateRequestParameters, CreateResponse, ICreateResponse>(
				request,
				cancellationToken,
				LowLevelDispatch.CreateDispatchAsync<CreateResponse>
			);
	}
}
