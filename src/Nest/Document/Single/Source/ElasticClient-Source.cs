...
namespace Nest
{
	...
	public partial class ElasticClient
	{
		...
		public T Source<T>(ISourceRequest request) where T : class
		{
			...
			var response = LowLevelDispatch.GetSourceDispatch<T>(request);
		...
		{
			request.RouteValues.Resolve(ConnectionSettings);
			var response = await LowLevelDispatch.GetSourceDispatchAsync<T>(request, cancellationToken).ConfigureAwait(false);
			return response.Body;
		}
	}
}
