...
namespace Nest
{
	public partial interface IElasticClient
	{
		...
		IObservable<IReindexResponse<T>> Reindex<T>(IndexName from, IndexName to, Func<ReindexDescriptor<T>, ReindexDescriptor<T>> reindexSelector = null)
			where T : class;
	...
	{
		/// <inheritdoc/>
		public IObservable<IReindexResponse<T>> Reindex<T>(IndexName from, IndexName to, Func<ReindexDescriptor<T>, ReindexDescriptor<T>> reindexSelector = null)
			where T : class => 
			this.Reindex<T>(reindexSelector.InvokeOrDefault(new ReindexDescriptor<T>(from, to)));
