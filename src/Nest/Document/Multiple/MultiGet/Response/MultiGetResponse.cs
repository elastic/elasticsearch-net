...
namespace Nest
{
	...
	{
		public IReadOnlyCollection<IMultiGetHit<object>> Documents => _Documents.ToList().AsReadOnly();
		public override bool IsValid => base.IsValid && !_Documents.HasAny(d => d.Error != null);

		internal ICollection<IMultiGetHit<object>> _Documents { get; set; } = new List<IMultiGetHit<object>>();

		public MultiGetHit<T> Get<T>(string id) where T : class => Documents.OfType<MultiGetHit<T>>().FirstOrDefault(m => m.Id == id);
		...
		{
			var docs = Documents.OfType<IMultiGetHit<T>>();
