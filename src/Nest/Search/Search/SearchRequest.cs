...
namespace Nest
{
	...
	/// </summary>
	[DataContract]
	public partial class SearchDescriptor<TDocument> where TDocument : class
	{
		protected override HttpMethod HttpMethod =>
			RequestState.RequestParameters?.ContainsQueryString("source") == true || RequestState.RequestParameters?.ContainsQueryString("q") == true
				? HttpMethod.GET
		...
		public SearchDescriptor<TDocument> Aggregations(Func<AggregationContainerDescriptor<TDocument>, IAggregationContainer> aggregationsSelector) =>
			Assign(aggregationsSelector(new AggregationContainerDescriptor<TDocument>())?.Aggregations, (a, v) => a.Aggregations = v);

		public SearchDescriptor<TDocument> Aggregations(AggregationDictionary aggregations) =>
			Assign(aggregations, (a, v) => a.Aggregations = v);

		public SearchDescriptor<TDocument> Source(bool enabled = true) => Assign(enabled, (a, v) => a.Source = v);

		public SearchDescriptor<TDocument> Source(Func<SourceFilterDescriptor<TDocument>, ISourceFilter> selector) =>
		...
		public SearchDescriptor<TDocument> Size(int? size) => Assign(size, (a, v) => a.Size = v);

		/// <summary>
		/// The number of hits to return. Alias for <see cref="Size" />. Defaults to 10.
		/// </summary>
		public SearchDescriptor<TDocument> Take(int? take) => Size(take);

		/// <summary>
		/// The starting from index of the hits to return. Defaults to 0.
		/// </summary>
		public SearchDescriptor<TDocument> From(int? from) => Assign(from, (a, v) => a.From = v);

		/// <summary>
		/// The starting from index of the hits to return. Alias for <see cref="From" />. Defaults to 0.
		/// </summary>
		public SearchDescriptor<TDocument> Skip(int? skip) => From(skip);
		...
		/// </summary>
		public SearchDescriptor<TDocument> Timeout(string timeout) => Assign(timeout, (a, v) => a.Timeout = v);

		/// <summary>
		/// Enables explanation for each hit on how its score was computed.
		/// (Use .DocumentsWithMetadata on the return results)
		/// </summary>
		public SearchDescriptor<TDocument> Explain(bool? explain = true) => Assign(explain, (a, v) => a.Explain = v);

		/// <summary>
		/// Returns a version for each search hit. (Use .DocumentsWithMetadata on the return results)
		/// </summary>
		public SearchDescriptor<TDocument> Version(bool? version = true) => Assign(version, (a, v) => a.Version = v);

		/// <summary>
		/// Make sure we keep calculating score even if we are sorting on a field.
		/// </summary>
		public SearchDescriptor<TDocument> TrackScores(bool? trackscores = true) => Assign(trackscores, (a, v) => a.TrackScores = v);
		...
		/// </summary>
		public SearchDescriptor<TDocument> Profile(bool? profile = true) => Assign(profile, (a, v) => a.Profile = v);

		/// <summary>
		/// Allows to filter out documents based on a minimum score:
		/// </summary>
		public SearchDescriptor<TDocument> MinScore(double? minScore) => Assign(minScore, (a, v) => a.MinScore = v);
		...
		/// terminated_early.
		/// </summary>
		public SearchDescriptor<TDocument> TerminateAfter(long? terminateAfter) => Assign(terminateAfter, (a, v) => a.TerminateAfter = v);
		...
		/// </para>
		/// </summary>
		public SearchDescriptor<TDocument> ExecuteOnPrimary() => Preference("_primary");
		...
		/// </para>
		/// </summary>
		public SearchDescriptor<TDocument> ExecuteOnPrimaryFirst() => Preference("_primary_first");
		...
		/// </para>
		/// </summary>
		public SearchDescriptor<TDocument> ExecuteOnLocalShard() => Preference("_local");
		...
		/// </para>
		/// </summary>
		public SearchDescriptor<TDocument> ExecuteOnNode(string node) => Preference($"_only_node:{node}");
		...
		/// </para>
		/// </summary>
		public SearchDescriptor<TDocument> ExecuteOnPreferredNode(string node) => Preference(node.IsNullOrEmpty() ? null : $"_prefer_node:{node}");
		...
		/// </summary>
		public SearchDescriptor<TDocument> IndicesBoost(Func<FluentDictionary<IndexName, double>, FluentDictionary<IndexName, double>> boost) =>
		...
		/// </summary>
		public SearchDescriptor<TDocument> StoredFields(Func<FieldsDescriptor<TDocument>, IPromise<Fields>> fields) =>
			Assign(fields, (a, v) => a.StoredFields = v?.Invoke(new FieldsDescriptor<TDocument>())?.Value);

		public SearchDescriptor<TDocument> StoredFields(Fields fields) => Assign(fields, (a, v) => a.StoredFields = v);

		public SearchDescriptor<TDocument> ScriptFields(Func<ScriptFieldsDescriptor, IPromise<IScriptFields>> selector) =>
			Assign(selector, (a, v) => a.ScriptFields = v?.Invoke(new ScriptFieldsDescriptor())?.Value);

		public SearchDescriptor<TDocument> DocValueFields(Func<FieldsDescriptor<TDocument>, IPromise<Fields>> fields) =>
			Assign(fields, (a, v) => a.DocValueFields = v?.Invoke(new FieldsDescriptor<TDocument>())?.Value);

		public SearchDescriptor<TDocument> DocValueFields(Fields fields) => Assign(fields, (a, v) => a.DocValueFields = v);

		/// <summary>
		/// A comma-separated list of fields to return as the field data representation of a field for each hit
		/// </summary>
		public SearchDescriptor<TDocument> Sort(Func<SortDescriptor<TDocument>, IPromise<IList<ISort>>> selector) =>
			Assign(selector, (a, v) => a.Sort = v?.Invoke(new SortDescriptor<TDocument>())?.Value);

		/// <summary>
		///  Sort values that can be used to start returning results "after" any document in the result list.
		/// </summary>
		public SearchDescriptor<TDocument> SearchAfter(IList<object> searchAfter) => Assign(searchAfter, (a, v) => a.SearchAfter = v);

		/// <summary>
		///  Sort values that can be used to start returning results "after" any document in the result list.
		/// </summary>
		public SearchDescriptor<TDocument> SearchAfter(params object[] searchAfter) => Assign(searchAfter, (a, v) => a.SearchAfter = v);

		/// <summary>
		///  The suggest feature suggests similar looking terms based on a provided text by using a suggester
		/// </summary>
		public SearchDescriptor<TDocument> Suggest(Func<SuggestContainerDescriptor<TDocument>, IPromise<ISuggestContainer>> selector) =>
			Assign(selector, (a, v) => a.Suggest = v?.Invoke(new SuggestContainerDescriptor<TDocument>())?.Value);

		/// <summary>
		/// Describe the query to perform using a query descriptor lambda
		/// </summary>
		public SearchDescriptor<TDocument> Query(Func<QueryContainerDescriptor<TDocument>, QueryContainer> query) =>
		...
		/// </summary>
		public SearchDescriptor<TDocument> Slice(Func<SlicedScrollDescriptor<TDocument>, ISlicedScroll> selector) =>
			Assign(selector, (a, v) => a.Slice = v?.Invoke(new SlicedScrollDescriptor<TDocument>()));

		/// <summary>
		/// Shortcut to default to a match all query
		/// </summary>
		public SearchDescriptor<TDocument> MatchAll(Func<MatchAllQueryDescriptor, IMatchAllQuery> selector = null) => Query(q => q.MatchAll(selector));

		/// <summary>
		/// Filter search using a filter descriptor lambda
		/// </summary>
		public SearchDescriptor<TDocument> PostFilter(Func<QueryContainerDescriptor<TDocument>, QueryContainer> filter) =>
			Assign(filter, (a, v) => a.PostFilter = v?.Invoke(new QueryContainerDescriptor<TDocument>()));
		...
		/// </summary>
		public SearchDescriptor<TDocument> Highlight(Func<HighlightDescriptor<TDocument>, IHighlight> highlightSelector) =>
			Assign(highlightSelector, (a, v) => a.Highlight = v?.Invoke(new HighlightDescriptor<TDocument>()));
		...
		/// </para>
		/// </summary>
		public SearchDescriptor<TDocument> Collapse(Func<FieldCollapseDescriptor<TDocument>, IFieldCollapse> collapseSelector) =>
			Assign(collapseSelector, (a, v) => a.Collapse = v?.Invoke(new FieldCollapseDescriptor<TDocument>()));

		/// <summary>
		/// Allows you to specify one or more queries to use for rescoring
		/// </summary>
		public SearchDescriptor<TDocument> Rescore(Func<RescoringDescriptor<TDocument>, IPromise<IList<IRescore>>> rescoreSelector) =>
			Assign(rescoreSelector, (a, v) => a.Rescore = v?.Invoke(new RescoringDescriptor<TDocument>()).Value);
	}
}
