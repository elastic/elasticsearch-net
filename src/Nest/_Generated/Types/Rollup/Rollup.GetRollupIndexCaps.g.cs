// Licensed to Elasticsearch B.V under one or more agreements.
// Elasticsearch B.V licenses this file to you under the Apache 2.0 License.
// See the LICENSE file in the project root for more information.
//
// ███╗   ██╗ ██████╗ ████████╗██╗ ██████╗███████╗
// ████╗  ██║██╔═══██╗╚══██╔══╝██║██╔════╝██╔════╝
// ██╔██╗ ██║██║   ██║   ██║   ██║██║     █████╗
// ██║╚██╗██║██║   ██║   ██║   ██║██║     ██╔══╝
// ██║ ╚████║╚██████╔╝   ██║   ██║╚██████╗███████╗
// ╚═╝  ╚═══╝ ╚═════╝    ╚═╝   ╚═╝ ╚═════╝╚══════╝
// ------------------------------------------------
//
// This file is automatically generated.
// Please do not edit these files manually.
//
// ------------------------------------------------

using Elastic.Transport.Products.Elasticsearch.Failures;
using OneOf;
using System;
using System.Collections.Generic;
using System.Text.Json.Serialization;

#nullable restore
namespace Nest.Rollup.GetRollupIndexCaps
{
	public partial class IndexCapabilities
	{
		[JsonPropertyName("rollup_jobs")]
		public IReadOnlyCollection<Nest.Rollup.GetRollupIndexCaps.RollupJobSummary> RollupJobs
		{
			get;
#if NET5_0
			init;
#else
			internal set;
#endif
		}
	}

	public partial class RollupJobSummary
	{
		[JsonPropertyName("fields")]
		public Dictionary<Nest.Field, IReadOnlyCollection<Nest.Rollup.GetRollupIndexCaps.RollupJobSummaryField>> Fields
		{
			get;
#if NET5_0
			init;
#else
			internal set;
#endif
		}

		[JsonPropertyName("index_pattern")]
		public string IndexPattern
		{
			get;
#if NET5_0
			init;
#else
			internal set;
#endif
		}

		[JsonPropertyName("job_id")]
		public Nest.Id JobId
		{
			get;
#if NET5_0
			init;
#else
			internal set;
#endif
		}

		[JsonPropertyName("rollup_index")]
		public Nest.IndexName RollupIndex
		{
			get;
#if NET5_0
			init;
#else
			internal set;
#endif
		}
	}

	public partial class RollupJobSummaryField
	{
		[JsonPropertyName("agg")]
		public string Agg
		{
			get;
#if NET5_0
			init;
#else
			internal set;
#endif
		}

		[JsonPropertyName("calendar_interval")]
		public Nest.Time? CalendarInterval
		{
			get;
#if NET5_0
			init;
#else
			internal set;
#endif
		}

		[JsonPropertyName("time_zone")]
		public string? TimeZone
		{
			get;
#if NET5_0
			init;
#else
			internal set;
#endif
		}
	}
}